...
def get_urls(save_to_file=False, read_from_file=True):
    if read_from_file and os.path.isfile("urls.txt"):
        return get_urls_from_file()
...
def get_distro_list():
    ...
    for distro_URL in get_urls()[3:]:
        ...
        based_on = distro_soup.find(
            "b", string="Based on:").parent.find_all("a")
        bases = []
        for base in based_on:
            bases.append(base.get_text())
        distro["based_on"] = bases

        desktops = distro_soup.find(
            "b", string="Desktop:").parent.find_all("a")
        desktop_names = []
